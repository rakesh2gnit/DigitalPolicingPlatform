package hcl.policing.digitalpolicingplatform.fragments.process.allegation;
...
public class AllegationFragment extends Fragment implements ItemClickListener {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        if (getArguments() != null) {
            ...
            allegationSelectionListener = (AllegationSelectionListener) getArguments().getSerializable("listener");
        }
        initView(view);
        return view;
    }
    ...
    private void setPersonPagerData() {
        if (allegationList != null && allegationList.size() > 0) {
            AllegationListAdaptor allegationPoleListAdaptor = new AllegationListAdaptor(mContext, allegationList, allegationSelectionListener, this);
            rvPersonList.setAdapter(allegationPoleListAdaptor);
        }
    }
    ...
    public void loadStopPersons() {
        ...
        new Thread(() -> {
            try {
                ...
                String strJson = JsonUtil.getInstance().loadJsonFromAsset(getActivity(), GenericConstant.ALLEGATION_POLE_LIST_JSON);
                EventResponse eventResponse = (EventResponse) JsonUtil.getInstance().toModel(strJson, EventResponse.class);
                if (eventResponse != null) {
                    allegationList = (ArrayList<OffenceDefinitionList>) eventResponse.getOffenceDefinitionList();
                }
                ...
                ExceptionLogger.Logger(e.getCause(), e.getMessage(), AllegationFragment.class, "loadStopPersons");
            }
        }).start();
    }
