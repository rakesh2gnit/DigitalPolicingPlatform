package hcl.policing.digitalpolicingplatform.fragments.process.fds.vehicle.pole;
...
public class VehiclePoleFragment extends Fragment implements ItemClickListener {

    private Context mContext;
    private RecyclerView rvPersonList;
    private ArrayList<VehicleList> aSearchVehicleListBean;
    ...
    private void setVehiclPagerData() {
        VehiclePNCListAdaptor vehiclePoleListAdaptor = new VehiclePNCListAdaptor(mContext, GenericConstant.TYPE_POLE,
                aSearchVehicleListBean, vehicleSelectionListener, this, isPopulate);
        rvPersonList.setAdapter(vehiclePoleListAdaptor);
    ...
    private void setDataToRead() {
        ...
        for (int i = 0; i < aSearchVehicleListBean.size(); i++) {
            if(i == 0) {
                textReadList.add(i+1 + "st vehicle VRM is " + aSearchVehicleListBean.get(i).getVRM() + ", make is " + aSearchVehicleListBean.get(i).getMake() + ", model is " + aSearchVehicleListBean.get(i).getModel() + " and color is " + aSearchVehicleListBean.get(i).getColor1() + ".");
            } else if(i == 1) {
                textReadList.add(i+1 + "nd vehicle VRM is " + aSearchVehicleListBean.get(i).getVRM() + ", make is " + aSearchVehicleListBean.get(i).getMake() + ", model is " + aSearchVehicleListBean.get(i).getModel() + " and color is " + aSearchVehicleListBean.get(i).getColor1() + ".");
            } else if (i == 2) {
                textReadList.add(i+1 + "rd vehicle VRM is " + aSearchVehicleListBean.get(i).getVRM() + ", make is " + aSearchVehicleListBean.get(i).getMake() + ", model is " + aSearchVehicleListBean.get(i).getModel() + " and color is " + aSearchVehicleListBean.get(i).getColor1() + ".");
            } else {
                textReadList.add(i+1 + "th vehicle VRM is " + aSearchVehicleListBean.get(i).getVRM() + ", make is " + aSearchVehicleListBean.get(i).getMake() + ", model is " + aSearchVehicleListBean.get(i).getModel() + " and color is " + aSearchVehicleListBean.get(i).getColor1() + ".");
            }
        }
     ...
     * Load Person JSON
     */
    public void loadPoleVehicle() {

        mProgressDialog = DialogUtil.showProgressDialog(getActivity());
        new Thread(() -> {
            try {

                String strJson = JsonUtil.getInstance().loadJsonFromAsset(getActivity(), GenericConstant.VEHICLE_POLE_LIST_JSON);
                PoleResponse poleResponse = (PoleResponse) JsonUtil.getInstance().toModel(strJson, PoleResponse.class);
                if (poleResponse != null) {
                    aSearchVehicleListBean = (ArrayList<VehicleList>) poleResponse.getPOLESearchResponse().getVehicleList();
                ...
                ExceptionLogger.Logger(e.getCause(), e.getMessage(), VehiclePoleFragment.class, "loadStopsVehicle");
            }
        }).start();
    }
    ...
    private void loadPollDetailsDialog() {
        ...
        PollDetailDialogFragment pollDetailDialogFragment = PollDetailDialogFragment.newInstance(GenericConstant.TYPE_VEHICLE, isPopulate);
    ...
    public void onItemClicked(int clickType) {
        SharedPrefUtils.getInstance(mContext).setString(SharedPrefUtils.Key.SYSTEM_NAME, GenericConstant.VEHICLE_POLE_DETAIL);
